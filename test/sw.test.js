...
const shellCache = "mandarin-taigi-shell-v11";
...
test("versioned dictionary data stays immutable with its app shell", async () => {
  const dataUrl = `${scope}data/dictionary.json?v=11`;
...
test("cached versioned shell assets are immutable for the worker lifetime", async () => {
  const appUrl = `${scope}app.js?v=11`;
...
test("quiz and learning modules are part of the install shell", () => {
  assert.match(workerSource, /"\.\/quiz\.js\?v=11"/);
  assert.match(workerSource, /"\.\/learning\.js\?v=11"/);
  assert.match(workerSource, /"\.\/offline\.js\?v=11"/);
});
