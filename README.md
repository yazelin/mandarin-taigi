...
- 可用華語詞目、臺灣台語漢字或臺羅搜尋；除官方「詞彙比較」外，也完整納入 5,548 筆「臺華共同詞」，並依官方釋義推導約 6,900 個華語詞的對照（釋義第一句是乾淨華語短詞才收，介面標示「依教育部釋義對照」，例：長頸鹿 → 長頷鹿、麒麟鹿）。
- 臺羅搜尋接受聲調符號或數字調，例如 `pīnn-īnn`、`pinn7-inn7` 都能找到「醫院」。
- 顯示教育部資料中的臺灣台語漢字、臺羅與 10 種腔口標示。
- 只有漢字與臺羅都能精確對上官方詞條時，才提供教育部原始台語發音。
- 274 個精確對上的單字詞優先使用教育部《國語辭典簡編本》原始「單字屬性聲音」WAV；未收錄的多字詞只使用裝置內建的本機華語 voice。
...
- 首次開啟會先查本機快取；沒有時才由固定資料版本的 CDN 下載約 1.02 MB 核心詞庫，CDN 不通則自動改由 GitHub Pages 取得。華語、台語漢字、臺羅與腔口搜尋會先開放；約 0.74 MB 的發音／來源／分類資料在背景補齊。通過版本與筆數驗證後，資料以獨立於介面版本的 cache key 留在本機，因此 UI-only 更新不會重抓同一版詞庫。
...
274 個華語單字音來自中華民國教育部（Ministry of Education, R.O.C.）《國語辭典簡編本》（版本 `2014_20260626`）：
